...
param_dist = {
    ...
    'min_child_weight': np.arange(1, 10), #9
    'gamma': np.linspace(0.0001, 0.0030, 30),
    'subsample': np.linspace(0.5, 1.0, 5), #0.9
    'colsample_bytree': np.linspace(0.5, 1.0, 5), #0.6
    'learning_rate': np.linspace(0.01, 0.20, 20), #0.06
...
x_columns = ['AR1_kNDVI', 'AR1_kNDVI_CV',
             'FC', 'FL', 'FG', 'IF', 'RF', 'PF',
...
x_train, x_test, y_train, y_test = train_test_split(x_all, y_all, test_size=0.2, random_state=603)
...
shap_values_df.to_csv(output_path+"shap_values_deltaTAC.csv", index=False)
...
# 保存SHAP特征重要性
shap_importance_df.to_csv(output_path+"shap_importance_deltaTAC.csv", index=False)
